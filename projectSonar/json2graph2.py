...
class JSON_EXE(object):
	...
	def cal_range2(self):
		...
		initial_test_range = self.averaging * 15
		...
		for i, self.envelop[i] in enumerate(self.envelop):
			...
			if start_index == 0:
				...
				if self.check_silent(i, tolerance_reflected + 10, tolerance + 3, saturation, 0, True):
					start_index_end = i
					break
	...
	def get_reflected_signal(self, emit_end, tolerance, saturation):
		...
		while i > emit_end and not found:
			...
			if self.check_slope(i - 2, i, tolerance - 2) == -1:
				...
				right_silent = self.check_silent(end, tolerance_range_right, tole_silent, saturation, 12, True)
				...
				left_silent = self.check_silent(start, tolerance_range_left, tole_silent * 1.5, saturation, False)
				if not left_silent:
	...
	def check_silent(self, i, tolerance_length=30, tolerance=5, saturation=51, offset=0, forward=True):
		start = 0
		end = 0
		if forward:
			start = i + offset
