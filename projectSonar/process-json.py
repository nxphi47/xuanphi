...
class JSONProcessor(object):
	...
	def cal_range2(self, data):
		...
		end_index_end = 0
		tolerance = 5
		emitting_envelop_min_diff = 100
		saturation = 0

		initial_test_range = self.settings['averaging'] * 15
		...
		for i, data['Envelop'][i] in enumerate(data['Envelop']):
			...
			if start_index == 0:
				...
				if self.check_silent(data, i, tolerance_reflected + 10, tolerance + 3, saturation, 0, True):
					start_index_end = i
					break
		...
		for i in xrange(len(data['Envelop']) - 1, start_index, -1):
			x = data['Envelop'][i]
			if x < saturation - lowerTole or x > saturation + emitting_envelop_min_diff + 50:
				# print "Fault: ID: ", data['Sample-ID'], "at ",i , " and val = ", x
				return 0, 0
	...
	def get_reflected_signal(self, data, emit_end, tolerance, saturation):
		...
		while i > emit_end and not found:
			...
			if self.check_slope(data, i - 2, i, tolerance - 2) == -1:
				...
				left_silent = self.check_silent(data, start, tolerance_range_left, tole_silent * 1.5, saturation, False)
				if not left_silent:
