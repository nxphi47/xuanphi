...
def get_sample_with_obj(inf, features, META=False, NUS=False):
	...
	if not NUS:
		...
		if 'refEnvList' not in features:
			features.append('refEnvList')
		if 'refTimeList' not in features:
			features.append('refTimeList')
		return {'fv': get_feature_vector(obj, features), 'id': obj['DataID'], 'actual_distance': _dist,
				'actual_height': _height, 'hazard': _hazard, 'surface': _surface, 'movement': _movement, 'label': label,
				'filename': obj['filename']}, obj
...
def get_sample_by_hazard(inf, hazard):
	"""return sample object based given hazard"""
	_obj = []
	for ins in inf.readlines():
		if ins == '':
			return None, None
...
def get_actual_dist_filename(filename):
	if 'm' in filename:
...
def get_curb_height(filename):
	if 'curb' in filename:
		...
		for x in name_label:
			if 'curb' in x:
				try:
					curb_height = float(x.replace('curb', ''))
				except ValueError, e:
					print "Could not find the curb height from filename: ", filename
					continue
				return curb_height
...
def guess_label_from_filename(filename, CLASS="Normal"):
	if CLASS == "Normal":
		...
		if 'curb' in filename:
			return 'C=' + str(int(get_curb_height(filename)))
			"""
			if get_curb_height(filename) < 15:
				return 'C<15'
			else:
				return 'C>15'
			"""
