...
class CommsHub (object):
	def __init__ (self, config, sock_handlers={}, stdin_handlers={}):
		...
		self.address = (self.config['raspi-addr'], self.config['udp-port'])
		self.udp = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
		self.debug("listening for UDP requests on port " + str(self.config['udp-port']))
	...
	def run (self):
		...
		thread.start_new_thread (comms_thread2, (self,))
	...
	def udp_send (self, cmd):
		if not self.udp is None:
			print "*** sending msg: %s" % cmd
			self.udp.sendto(cmd+'\0', self.address)
		else:
			print "*** ERROR sending msg: %s" % cmd
	
	def udp_get (self):
		if not self.udp is None:
			return self.udp.recvfrom(4096)
		else:
			return None, None
				
	def comms_loop2 (self):
		while not self.stop:
			rfds, wfds, efds = select.select([self.udp, sys.stdin], [], [], 3.0)
			if self.udp in rfds:
				msg, server = self.udp_get()
				#self.debug("*** received %s response: %s" % (repr(server), repr(msg)))
				res = self.sock_handlers['UDP'] (self.udp, msg)
		...
		self.udp.sendto('QUIT\0', server_address)
		self.udp.close()
