...
class singleReflect (object):
	...
	def process_data(self):
		...
		print '\n*************** %d ***************' % self.index
		print 'Pulse Width: %d, Time Width: %.3f, Saturate: %d'% (self.data[self.pulseWidth], self.data[self.timeWidth], self.saturate)
	...
	def get_peak(self, rev=None):
		...
		print 'Peak: %d, Min: %d, PeakTime: %.3f' % (self.data[self.peakValue], self.data[self.minValue], self.data[self.peakTime])
		...
		for x in xrange(len(group)):
			if x == range_id:
				...
				if (len(group[x]) % 2) == 0:
					...
					_time = (self.data[self.refTimeList][_second_index] + self.data[self.refTimeList][_first_index]) / 2.
					if DEBUG == 1:
				...
				_range_area2 = self.cal_area((self.data[self.refTimeList][_act_count - 1 + len(group[x])]- _time), _env, _env)
	...
	def get_area(self):
		...
		if DEBUG == 1:
			...
			self.data[self.refTimeList] = [21.12, 21.168, 21.216, 21.264, 21.312, 21.36, 21.408, 21.456, 21.504, 21.552, 21.6, 21.648, 21.696, 21.744, 21.792, 21.84, 21.888, 21.936, 21.984]
