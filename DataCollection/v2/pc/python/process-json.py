...
class JSONProcessor(object):
	def __init__(self, config, _dict, root):
		...
		self.settings['test'] = convert_cfg(get_cfg(self.cfg, 'test', 'True'))
	...
	def load_file(self, filename):
		...
		jsonWholeObject = eval(fileString)

		# extracting object and process
		for count in xrange(len(jsonWholeObject)):
			obj = jsonWholeObject[count]
			data = self.process_object(obj)		# change for AIS
			if data:
				data['filename'] = filename
				self.data_id += 1
				data['DataID'] = self.data_id
				ret.append(data)
				print "Sample ID: %d - OK" %(data['DataID'])
				if self.settings['output_graph']:
					self.plot_graph(data)
			else:
				print "Count %d : None" %(count)
	...
	def process_object(self, obj):
		...
		if obj.has_key("sensors"):
			...
			data = {'Timing': [], 'Envelop': []}
			if obj.has_key('start'):
				data['Start-Time'] = [obj['start'][0], obj['start'][1]]
			...
			for i in xrange(len(obj['timing'])):
				...
				t += td
				data['Timing'].append(t)
				data['Envelop'].append(float(obj['envelop'][i]))
			...
			if (not _min == 0) and (not _max == 0):
				...
				if self.settings['test']:
					...
					if dist is not None:
						if not (dist + tolerance > self.get_dist(data['Timing'][_min],
																 data['Timing'][_max]) > dist - tolerance):
							print "Real dist not same as filename dist"
							return None
	...
	def get_dist(self, xmin, xmax):
		_sound_speed = 336.4 + .61 * (self.settings['temperature'])
	...
	def cal_range2(self, data):
		...
		end_index_end = 0
		tolerance = 75
		emitting_envelop_min_diff = 1100
		saturation = 0

		initial_test_range = self.settings['averaging'] * 10
	...
	def get_reflected_signal(self, data, emit_end, tolerance, saturation):
		...
		tolerance_range_signal = 30	# length of the signal itself, which should not exceed this value
		offset = 10
		...
		while i > emit_end + 10:
			...
			if self.check_slope(data, i - 1, i, tolerance - 25) == -1:
				...
				left_silent = self.check_silent(data, start, tolerance_range_left, tole_silent * 1.5, saturation, offset, False)
				if not left_silent:
