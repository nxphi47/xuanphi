...
#ifdef __VER_3_CIRCUIT__
/* using version 3.0 circuit */

// address pins
#define RPIGPIO_A0		12
#define RPIGPIO_A1		13
#define RPIGPIO_ACS		10
#define RPIGPIO_ARX		9
#define RPIGPIO_RDC		23
#define RPIGPIO_BYTE	24

#define RPIGPIO_A0_bit		0x00001000	// 1<<12
#define RPIGPIO_A1_bit		0x00002000	// 1<<13
#define RPIGPIO_ACS_bit		0x00000400	// 1<<10
#define RPIGPIO_ARX_bit		0x00000200	// 1<<9
#define RPIGPIO_RDC_bit		0x00800000	// 1<<23
#define RPIGPIO_BYTE_bit	0x01000000	// 1<<24

// data pins
#define RPIGPIO_D0		5
#define RPIGPIO_D1		6
#define RPIGPIO_D2		17
#define RPIGPIO_D3		18
#define RPIGPIO_D4		19
#define RPIGPIO_D5		20
#define RPIGPIO_D6		21
#define RPIGPIO_D7		22

#define RPIGPIO_D0_bit		0x00000020	// 1<<5
#define RPIGPIO_D1_bit		0x00000040	// 1<<6
#define RPIGPIO_D2_bit		0x00020000	// 1<<17
#define RPIGPIO_D3_bit		0x00040000	// 1<<18
#define RPIGPIO_D4_bit		0x00080000	// 1<<19
#define RPIGPIO_D5_bit		0x00100000	// 1<<20
#define RPIGPIO_D6_bit		0x00200000	// 1<<21
#define RPIGPIO_D7_bit		0x00400000	// 1<<22

#define RPIGPIO_D0_shift	4
#define RPIGPIO_D1_shift	4
#define RPIGPIO_D2_shift	16
#define RPIGPIO_D3_shift	16
#define RPIGPIO_D4_shift	16
#define RPIGPIO_D5_shift	16
#define RPIGPIO_D6_shift	16
#define RPIGPIO_D7_shift	16

#define RPIGPIO_D01_bit			0x00000060
#define RPIGPIO_D01_shift		4
#define RPIGPIO_D234567_bit		0x007E0000
#define RPIGPIO_D234567_shift	16

#define RPIGPIO_BUSY	25

#define RPIGPIO_BUSY_bit	0x02000000	// 1<<25

#endif // __VER_3_CIRCUIT__
...
RPI_CB_p rpigpio_init (void)
{
	if (!__rpicb) {
		...
		__CONFIGURE_GPIO(RPIGPIO_A0, RPIGPIO_OUTPUT, RPIGPIO_PUD_OFF);
		__CONFIGURE_GPIO(RPIGPIO_A1, RPIGPIO_OUTPUT, RPIGPIO_PUD_OFF);
		__CONFIGURE_GPIO(RPIGPIO_ACS, RPIGPIO_OUTPUT, RPIGPIO_PUD_OFF);
		__CONFIGURE_GPIO(RPIGPIO_ARX, RPIGPIO_OUTPUT, RPIGPIO_PUD_OFF);
		__CONFIGURE_GPIO(RPIGPIO_RDC, RPIGPIO_OUTPUT, RPIGPIO_PUD_OFF);
		__CONFIGURE_GPIO(RPIGPIO_BYTE, RPIGPIO_OUTPUT, RPIGPIO_PUD_OFF);
		__CONFIGURE_GPIO(RPIGPIO_D0, RPIGPIO_INPUT, RPIGPIO_PUD_DOWN);
		__CONFIGURE_GPIO(RPIGPIO_D1, RPIGPIO_INPUT, RPIGPIO_PUD_DOWN);
		__CONFIGURE_GPIO(RPIGPIO_D2, RPIGPIO_INPUT, RPIGPIO_PUD_DOWN);
		__CONFIGURE_GPIO(RPIGPIO_D3, RPIGPIO_INPUT, RPIGPIO_PUD_DOWN);
		...
		__CONFIGURE_GPIO(RPIGPIO_D7, RPIGPIO_INPUT, RPIGPIO_PUD_DOWN);
		__CONFIGURE_GPIO(RPIGPIO_BUSY, RPIGPIO_INPUT, RPIGPIO_PUD_DOWN);
		gpioWrite(RPIGPIO_ARX, 1);	// disable ARX
#endif
...
// Ranging is ARX=0
#define __range__() \
	gpioSetBank1(RPIGPIO_ARX_bit); \
	gpioClearBank1(RPIGPIO_RDC_bit | RPIGPIO_ACS_bit | RPIGPIO_ARX_bit | RPIGPIO_BYTE_bit)

// start conversion, assuming A0, A1 is already set
// ADC conversion is ACS=0, RDC=0
#define __convert__() \
	gpioSetBank1(RPIGPIO_RDC_bit | RPIGPIO_ACS_bit | RPIGPIO_ARX_bit); \
	gpioClearBank1(RPIGPIO_ARX_bit)

// latch the converted data to the data bus
// latch low byte data first
// ADC latch is ACS=0, RDC=1, BYTE=0
#define __latch1__() \
	gpioSetBank1(RPIGPIO_ACS_bit); \
	gpioClearBank1(RPIGPIO_ARX_bit | RPIGPIO_RDC_bit | RPIGPIO_BYTE_bit); \
	rpigpio_pins_show();

// latch the converted data to the data bus
// latch low byte data first
// ADC latch is ACS=0, RDC=1, BYTE=1
#define __latch2__() \
	gpioSetBank1(RPIGPIO_BYTE_bit); \
	gpioClearBank1(RPIGPIO_ARX_bit); \
	rpigpio_pins_show();
#endif
...
#elif __VER_3_CIRCUIT__
	if ((id >= 0) && (id < NUM_SENSORS)) {
		gpioSetBank1(__rpicb->sensors[id].A0A1_bits_set | RPIGPIO_ARX_bit);
		gpioClearBank1(__rpicb->sensors[id].A0A1_bits_clr);
		__range__();
	}
#endif
...
#define __read_byte__(R) {	\
	uint32_t reg = gpioReadBank1();	\
	R |= (reg & RPIGPIO_D01_bit) >> RPIGPIO_D01_shift; \
	R |= (reg & RPIGPIO_D234567_bit) >> RPIGPIO_D234567_shift;	\
}
#endif
...
void rpigpio_pins_show (void)
{
...
#elif __VER_3_CIRCUIT__
	uint32_t A0, A1, ACS, ARX, RDC, BYTE, D0, D1, D2, D3, D4, D5, D6, D7, BUSY;
	A0 = (reg & RPIGPIO_A0_bit) ? 1 : 0;
	A1 = (reg & RPIGPIO_A1_bit) ? 1 : 0;
	ACS = (reg & RPIGPIO_ACS_bit) ? 1 : 0;
	ARX = (reg & RPIGPIO_ARX_bit) ? 1 : 0;
	RDC = (reg & RPIGPIO_RDC_bit) ? 1 : 0;
	BYTE = (reg & RPIGPIO_BYTE_bit) ? 1 : 0;
	D0 = (reg & RPIGPIO_D0_bit) ? 1 : 0;
	D1 = (reg & RPIGPIO_D1_bit) ? 1 : 0;
	D2 = (reg & RPIGPIO_D2_bit) ? 1 : 0;
	D3 = (reg & RPIGPIO_D3_bit) ? 1 : 0;
	...
	D7 = (reg & RPIGPIO_D7_bit) ? 1 : 0;
	BUSY = (reg & RPIGPIO_BUSY_bit) ? 1 : 0;
	printf("A0[%d] A1[%d] ACS[%d] ARX[%d] RDC[%d] BYTE[%d] D[%d%d%d%d %d%d%d%d] BUSY[%d]\n",
		A0, A1, ACS, ARX, RDC, BYTE, D7, D6, D5, D4, D3, D2, D1, D0, BUSY);
#endif
}
...
#elif __VER_3_CIRCUIT__
		// init address bus
		gpioSetBank1(sen->A0A1_bits_set | RPIGPIO_ARX_bit);
		gpioClearBank1(sen->A0A1_bits_clr);
#endif
...
#elif __VER_3_CIRCUIT__
		gpioSetBank1(RPIGPIO_ARX_bit);	// disable ARX
#endif
...
#ifdef __VER_2_CIRCUIT__
			gpioSetBank1(senp->A0A1_bits_set | RPIGPIO_ALE_bit);
			gpioClearBank1(senp->A0A1_bits_clr);
#elif __VER_3_CIRCUIT__
			// init address bus
			gpioSetBank1(sen->A0A1_bits_set | RPIGPIO_ARX_bit);
			gpioClearBank1(sen->A0A1_bits_clr);
...
const char * rpigpio_history_print (SHISTORY_p hist, int senid)
{
	...
	p += sprintf (p, "{  'start': [%u, %u], 'len': %d, 'recv-sensor': %d, 'firing-sensor': %d,\n  'samples': [", 
			hist->sec_start_time, hist->usec_start_time % 1000000, hist->num_samples, senid, hist->sensorid);
