...
void usage (char * progname) {
	printf ("\nUsage:\n\t%s [-n <num-cycles>] [-s <enabled-sensors>] [-t <timings>] [-o <filename>] [-a]\n", progname);
...
int main (int argc, char * argv[])
{
	...
	int i=1, j=0, k=0, ret, num_cycle=1, all=0;
	char * fname = NULL;
	const char * buf;
	SENSOR_p sen;
	SHISTORY_p pSam;
	...
	for (i=0; i<num_cycle; i++) {
		for (j=0, sen=rpi->sensors; j<NUM_SENSORS; j++, sen++) {
			printf("\nRanging cycle %d: Sensor#%d\n", i, j);
			if (!all) {
				rpigpio_sensor_range_and_sample (sen);
				rpigpio_sensor_update_history(sen, 0);
			} else {
				rpigpio_sensor_range_and_sample_all (sen);
				for (k=0; k<NUM_SENSORS; k++)
					rpigpio_sensor_update_history(rpi->sensors+k, 0);
			}
		}
	}
